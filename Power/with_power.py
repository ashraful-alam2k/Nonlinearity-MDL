...
def create_dataset_m(
    ...
    dataset_x_pol__des = np.empty([dataset_size, 2], dtype="float64")  # uninitialized
    dataset_x_pol__des[:, 0] = np.real(x_pol_des_complex[dataset_range])  # all rows and 1st column
    dataset_x_pol__des[:, 1] = np.imag(x_pol_des_complex[dataset_range])
    dataset_x_pol__in = np.empty([dataset_size, n_sym1, 5], dtype="float64")
    dataset_x_pol__in[:] = np.nan  # all the rows and column
...
power_train = np.random.randint(-1, 6, training_epochs)
power_train_reshape = power_train.reshape(-1, 1)
scaler = StandardScaler()
scaler.fit(power_train_reshape)
power_train_standardized = scaler.transform(power_train_reshape)
#
# # Following lines are used when using a saved model
model_w_aug = keras.models.load_model("with_power_feature_tap70_layer4_span50_SR64_550.h5")
model_w_aug.summary()
#
# NN model
inputs = tf.keras.Input(shape=(n_sym, 5), name="digits")  # (Input is n_sym, 5 dimensional vector)
...
for epoch in range(training_epochs):
    ...
    dataset_x_pol_in_train, dataset_x_pol_des_train = create_dataset_m(
        ...
        n_sym,
        power_train_standardized[epoch],
    )
    ...
    print("Epoch now", epoch,
          "Power now", power_train[epoch])
    print(
        "Q_train Sim now", Q_train_predict,
        "Q_train Sim ref now", Q_train_ref
    )
    model_w_aug.save("with_power_feature_tap70_layer4_span50_SR64_750.h5")
#
...
power_test_reshape = power_test.reshape(-1, 1)
power_test_standardized = scaler.transform(power_test_reshape)
Q_prop_ref3, Q_prop_ref4 = np.zeros(len(power_test)), np.zeros(
    len(power_test)
)
...
for epoch in range(test_epochs):
    ...
    savedModel = keras.models.load_model("with_power_feature_tap70_layer4_span50_SR64_750.h5")
    ...
    print("Power now", power_test[epoch])
    print(
        "Q_test Exp now", Q_prop_ref3[epoch],
        "Q-factor  Exp ref", Q_prop_ref4[epoch],
    )

np.save("with_power_feature_tap70_layer4_span50_SR64_750", Q_prop_ref3)
np.save("with_power_feature_tap70_layer4_span50_SR64_750_ref", Q_prop_ref4)


plt.plot(power_test, Q_prop_ref3, label="Exp now NN", c="blue")
plt.plot(power_test, Q_prop_ref4, label="Exp ref no NN", c="red")
plt.title("with_power_feature_tap70_layer4_span50_SR64_750")
plt.legend()
plt.grid()
plt.show()
