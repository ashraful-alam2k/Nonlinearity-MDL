...
# model_w_aug = keras.models.load_model("model_without_span_feature2.h5")
# model_w_aug.summary()
...
#         n_spans=int(n_span_train[epoch]),
#         z_span=fiber_length,
#         alpha_db=alpha,
#         gamma=gamma,
#         noise_figure_db=NF,
...
#     )
#     print("Epoch now", epoch, "Span now", n_span_train[epoch])
#     print(
#         "Q_train Sim now", Q_train_predict,
#         "Q_train Sim ref now", Q_train_ref
#     )
#     model_w_aug.save("without_span_feature_power8_1000.h5")

n_span_test = [x for x in range(10, 65, 5)]
test_epochs = len(n_span_test)
Q_prop_ref3, Q_prop_ref4 = np.zeros(len(n_span_test)),\
                           np.zeros(len(n_span_test))

for epoch in range(test_epochs):
    channel = ch.create_channel_parameters(
        n_spans=int(n_span_test[epoch]),
        z_span=fiber_length,
        alpha_db=alpha,
        gamma=gamma,
        noise_figure_db=NF,
    ...
    savedModel = keras.models.load_model("model_without_span_feature1300.h5")
...
np.save("model_without_span_feature_power6_60", Q_prop_ref3)
np.save("model_without_span_feature_power6_60_ref", Q_prop_ref4)
...
plt.xticks(x)
plt.yticks(y)
plt.title("model_without_span_feature_power6_60")
plt.show()
