...
def create_dataset_m(
    ...
    for vec_idx, center_vec in enumerate(dataset_range):
        ...
        if np.any(local_range < 0) or np.any(local_range > raw_size):
            ...
            dataset_x_pol__in[vec_idx, n1, 4] = n_span
...
n_span_train = np.random.randint(10, 51, training_epochs)
...
model_w_aug = keras.models.load_model("with_span_feature_layer4_tap70_epoch1300.h5")
model_w_aug.summary()

# # NN model
# inputs = tf.keras.Input(shape=(n_sym, 5), name="digits")
...
#           "Span now", n_span_train[epoch])
#     print(
#         "Q_train Sim now", Q_train_predict,
#         "Q_train Sim ref now", Q_train_ref
#     )
#     model_w_aug.save("with_span_feature_layer4_tap70_epoch1000.h5")

n_span_test = [x for x in range(10, 65, 5)]
test_epochs = len(n_span_test)
...
Q_prop_ref3, Q_prop_ref4 = np.zeros(len(n_span_test)), np.zeros(
    len(n_span_test)
)
...
for epoch in range(test_epochs):
    ...
    savedModel = keras.models.load_model("with_span_feature_layer4_tap70_epoch1300.h5")
    ...
    print("Span now", n_span_test[epoch])
    print(
        "Q_test Exp now", Q_prop_ref3[epoch],
        "Q-factor  Exp ref", Q_prop_ref4[epoch],
    )

np.save("with_span_feature_power6_60", Q_prop_ref3)
np.save("with_span_feature_power6_60_ref", Q_prop_ref4)

plt.plot(n_span_test, Q_prop_ref3, label="Exp now NN", c="blue")
plt.plot(n_span_test, Q_prop_ref4, label="Exp ref no NN", c="red")
plt.title("with_span_feature_power6_60")
plt.legend()
plt.grid()
plt.show()
