...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Set the currently used GPU device to be only the device name of the 0/gpu:0'
print(tf.config.list_physical_devices("GPU"))
# Set seed for reproducibility
seed = 123
os.environ["PYTHONHASHSEED"] = str(seed)  # https://ryansblog.xyz/post/3a1b00b7-e4a5-4f0a-9742-05f3171ad317
random.seed(seed)
...
# model_w_aug = keras.models.load_model('model_without_SR_feature_1000.h5')
# model_w_aug.summary()
...
for epoch in range(training_epochs):
    ...
    print("Epoch now", epoch, "SR now", symbol_rate_train[epoch])
    print("Q_train Sim now", Q_train_predict, "Q_train Sim ref now", Q_train_ref)
    model_w_aug.save("model_without_SR_feature_power6.h5")
print(
...
for epoch in range(test_epochs):
    ...
    savedModel = keras.models.load_model("model_without_SR_feature_power6.h5")
...
np.save("without_SR_feature_power6", Q_prop_ref3)
np.save("without_SR_feature_power6_ref", Q_prop_ref4)
...
plt.legend()
plt.title("model_without_SR_feature_power6")
plt.show()
