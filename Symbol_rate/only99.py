...
symbol_rate_train = 99  # np.random.uniform(40, 100, training_epochs)
# NN
...
# This is needed when training from a saved model
# model_w_aug = keras.models.load_model(<saved_model_name>)
# model_w_aug.summary()
#
exception_train = []
for epoch in range(training_epochs):
    ...
    model_w_aug.save("model_only99_power6.h5")


symbol_rate_test = [x for x in range(40, 100, 1)]

test_epochs = len(symbol_rate_test)
Q_prop_ref3, Q_prop_ref4 = np.zeros(len(symbol_rate_test)), \
                           np.zeros(len(symbol_rate_test))
# exception_test = []
for epoch in range(test_epochs):
    ...
    savedModel = keras.models.load_model("model_only99_power6.h5")
...
np.save("only99_power6", Q_prop_ref3)
np.save("only99_power6_ref", Q_prop_ref4)
